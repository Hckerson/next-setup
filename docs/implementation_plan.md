...
### `lib/interface/contest.ts` (Competition / Contests Alignment)

- The structures for `Competition` and `Contests` are deeply intertwined but use `EventData` differently (`Contests` extends it, `Competition` wraps it). I will ensure `EventData` accurately represents the shared base layer between both data models and remove any loose re-declarations.
- Ensure `RecommendedProfile` in `user.ts` is extending `Competition` securely without missing metadata.
